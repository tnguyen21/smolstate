...
def train_command(args):
    ...
    updated_model_config = DataConfig.update_model_config_with_data_dims(
        config.config["model"], model_dims
    )
...
def main():
    ...
    train_parser.add_argument(
        "--name",
        default="smolstate_run",
        help="Experiment name"
    )
    train_parser.add_argument(
        "--state-path",
        default="../state",
        help="Path to state repository"
    )
    train_parser.add_argument(
        "--resume",
    ...
    train_parser.add_argument(
        "--no-resume",
        action="store_true",
        help="Don't auto-resume from latest checkpoint"
    )
    train_parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    train_parser.add_argument(
        "overrides",
        nargs="*",
        help="Configuration overrides (e.g., training.max_steps=1000)"
    )
