...
class SimplePerturbationDataset:
    ...
    def _load_metadata(self):
        ...
        with h5py.File(self.h5_path, 'r') as f:
            ...
            self.cell_type_codes = f[f"obs/{self.cell_type_key}/codes"][:] if f"obs/{self.cell_type_key}/codes" in f else f[f"obs/{self.cell_type_key}"][:]
            self.batch_codes = f[f"obs/{self.batch_col}/codes"][:] if f"obs/{self.batch_col}/codes" in f else f[f"obs/{self.batch_col}"][:]
...
class PerturbationDataModule:
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"Using columns: pert_col={self.pert_col}, batch_col={self.batch_col}, cell_type_key={self.cell_type_key}, control_pert={self.control_pert}")
    ...
    def _setup_global_maps(self):
        ...
        logger.info(f"Created maps: {len(all_perts)} perts, {len(all_batches)} batches, {len(all_celltypes)} cell types")
